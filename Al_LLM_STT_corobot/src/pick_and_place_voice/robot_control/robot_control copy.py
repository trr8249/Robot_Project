...
class RobotController(Node):
    ...
    def robot_control(self):
        ...
        # ✅ 여기부터 수정 포인트: LLM 결과 파싱
        result_str = get_keyword_future.result().message
        try:
            object_str, target_str, amount = result_str.split("/")
        except ValueError:
            self.get_logger().error("Invalid format returned from keyword service.")
            return

        object_list = object_str.strip().split()
        target_list = target_str.strip().split()
        amount = amount.strip()

        # ✅ 아무것도 없을 경우
        if len(object_list) == 0 and len(target_list) == 0:
    ...
    def pump_moving(self, target_pos):
...
# 3. 주유구에서 카드 보이는 위치로 이동
    def card_moving(self, target_pos):
        movel(posx(0.0, 0.0, 130.0, 0, 0, 0), vel=VELOCITY, acc=ACC, radius=0.0, ref=DR_BASE, mod=DR_MV_MOD_REL)
        gripper.close_gripper()
        mwait()
    ...
    #     print("Moving to JOne...")
    #     JOne = [-50.02, 49.38, 25.28, 45.19, 111.30, -16.49]
    #     movej(JOne, vel=VELOCITY, acc=ACC)
    #     mwait()
    #     movel(target_pos, vel=VELOCITY, acc=ACC)
    #     mwait()
    ...
    #     print(f"\n:흰색_확인_표시: Reached target position:")
